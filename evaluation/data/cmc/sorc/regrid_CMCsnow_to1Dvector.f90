program data_regrid
  ...
  namelist/regrid_cmc_nml/ destination_locs, weight_locs, fv3_grid, source_path, weights_filename, destination_path
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  destination_filename=trim(destination_path)//trim(fv3_grid)//'_cmc_snow_depth_'//idate//'.nc'
...
!!!! if source_input values exist, do regrid work !!!
  ...
  !write(*,'(192(1x,f5.1))') cmcSnowDepth 
...
! Define variables in the file.
  ...
  status = nf90_def_var(ncid, "snowDepth", NF90_FLOAT, (/dim_id_i, dim_id_t/), varid)
    if (status /= nf90_noerr) call handle_err(status)
...
! Write variables in the file.
  ...
  status = nf90_inq_varid(ncid, "snowDepth", varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_var(ncid, varid , cmcSnowDepth, start = (/1,1/), count = (/destination_locs,1/)) 
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_att(ncid, varid, "_FillValue", fillVal)
    if (status /= nf90_noerr) call handle_err(status)
